...
from bot import Bot
from config import OWNER_ID, FORCE_MSG, START_MSG, CUSTOM_CAPTION, DISABLE_CHANNEL_BUTTON, PROTECT_CONTENT
from helper_func import subscribed, encode, decode, get_messages
from database.database import add_user, del_user, full_userbase, present_user

# A list to store authorized users
AUTHORIZED_USERS = []
...
async def start_command(client: Client, message: Message):
    ...
    # If the user is authorized or is the owner, proceed
...
# Admin command to authorize users
@Bot.on_message(filters.command("authorize") & filters.private & filters.user(OWNER_ID))
async def authorize_user(client, message):
    if message.reply_to_message:
        user_to_authorize = message.reply_to_message.from_user.id
        if user_to_authorize not in AUTHORIZED_USERS:
            AUTHORIZED_USERS.append(user_to_authorize)
            await message.reply_text(f"✅ User {user_to_authorize} has been authorized!")
        else:
            await message.reply_text("⚠️ User is already authorized.")
    else:
        await message.reply_text("Please reply to a user's message to authorize them.")
